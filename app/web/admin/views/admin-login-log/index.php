    ...
    <?php
        echo GridView::widget([
            ...
            'columns'   => [
                ...
                ['attribute' => 'all_result', 'format' => 'raw', 'value' => function($model) {
                    if ($model->all_result == AdminLoginLog::RESULT_OK) {
                        return '<span class="btn btn-success btn-sm" disabled="disabled">'. AdminLoginLogService::getResultText($model->all_result) .'</span>';
                    } elseif ($model->all_result == AdminLoginLog::RESULT_FAILD) {
                        return '<span class="btn btn-warning  btn-sm" disabled="disabled">'. AdminLoginLogService::getResultText($model->all_result) .'</span>';
                    }
                    return '-';
                }],
                ['attribute' => 'all_content'],
                ['attribute' => 'all_addtime', 'value' => function($model) {
                     return date('Y-m-d H:i', $model->all_addtime);
                }],
                ['attribute' => 'all_addip', 'value' => function($model) {
                     return long2ip($model->all_addip);
                }],
            ],
            'pager' => [
...
$(document).ready(function(){
    /* 初始化 */
    CLS_FORM.init({url: "<?php echo Url::to(['admin-login-log/index']); ?>"});

    /* 时间控件 */
    $.datetimepicker.setLocale('zh');
    $('#adminloginlog-add_start_date').datetimepicker({
        format: 'Y-m-d',
        timepicker:false,
        todayButton: true,
        onShow:function(ct){
            this.setOptions({
                maxDate: $('#adminloginlog-add_end_date').val() ? $('#adminloginlog-add_end_date').val() : false
            })
       },
    });
    /* 时间控件 */
    $('#adminloginlog-add_end_date').datetimepicker({
        format: 'Y-m-d',
        timepicker:false,
        todayButton: true,
        onShow:function(ct){
            this.setOptions({
                minDate: $('#adminloginlog-add_start_date').val() ? $('#adminloginlog-add_start_date').val() : false
            })
       },
    });
    ...
    $('button[adm="submit"]').on('click', function(){
        var _ad_id    = $('#adminloginlog-ad_id').val();
        var _username = $('#adminloginlog-username').val();
        var _name     = $('#adminloginlog-name').val();
        var _result   = $('#adminloginlog-result option:checked').val();
        var _add_start_date = $('#adminloginlog-add_start_date').val();
        var _add_end_date   = $('#adminloginlog-add_end_date').val();
        var _data = {'AdminLoginLog[ad_id]': _ad_id, 'AdminLoginLog[username]': _username, 'AdminLoginLog[name]': _name, 'AdminLoginLog[result]': _result, 'AdminLoginLog[add_start_date]': _add_start_date, 'AdminLoginLog[add_end_date]': _add_end_date};
